...
const pubsub = new PubSub({
  projectId: 'sunny-vehicle-436819-f5',
   keyFilename: './ewallet-function-keys.json'
}
);
...
const allowedOrigins = [
  "http://localhost:4200",
  "https://e-wallet-fe-302717077924.us-central1.run.app",
];
...
app.post("/users", async (req, res) => {
  let { first_name, last_name, email, password, role, account_status } =
    req.body;
  ...
  try {
    ...
    const createWalletResult = await executeQuery(
      createWalletclient,
      createWalletQuery,
      createWalletValues
    );
  ...
  try {
    ...
    res
      ...
      .send({
        ...
        transactionData: transactionData,
        senderBalance: postsenderBalance,
      });
...
app.get("/getTransactions", async (req, res) => {
  ...
  try {
    const client = await connectToDatabase();
    const query =
      "SELECT * FROM transactions WHERE sender_wallet = $1 OR receiver_wallet = $1 ORDER BY id DESC LIMIT 10";
    const values = [id];
    const result = await executeQuery(client, query, values);
...
app.post("/fundAccount", async (req, res) => {
  ...
  try {
    ...
    if (parseFloat(poolBalance) < parseFloat(amount)) {
      res
        .status(400)
        .send({ message: "Insufficient Funds with Internal Account" });
      return;
    }
    ...
    res
      ...
      .send({
        ...
        transactionData: transactionData,
        newBalance: postrecipientBalance,
      });
...
app.get("/filterTransactions", async (req, res) => {
  ...
  try {
    const client = await connectToDatabase();
    const query =
      "SELECT * FROM transactions WHERE sender_wallet = $1 OR receiver_wallet = $1 ORDER BY id";
    const values = [id];
    ...
    const wallet_result = await executeQuery(
      wallet_client,
      wallet_query,
      wallet_values
    );
    ...
    if (transaction_result === null || wallet_result === null) {
      ...
      const creditCount = transaction_result.rows.filter(
        (transaction) => transaction.transaction_type === "Credit"
      ).length;
      const debitCount = transaction_result.rows.filter(
...
async function getBalance(client, walletId) {
  console.log("Getting balance for wallet ID:", walletId);
...
async function getUserInfo(client, email, wallet_id) {
  const query =
    "SELECT id,first_name,last_name,email FROM users WHERE email = $1 OR wallet_id = $2";
...
async function sendEmail(emailData) {
  var client = await connectToDatabase();
  const recipientInfo = await getUserInfo(
    client,
    "",
    emailData.recipient_wallet
  );
...
async function executeMail(emailData, template) {
